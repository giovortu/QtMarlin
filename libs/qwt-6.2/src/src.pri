...
contains(QWT_CONFIG, QwtSvg) {
    ...
    greaterThan(QT_MAJOR_VERSION, 4) {
        ...
        qtHaveModule(svg) {
            ...
            warning("QwtSvg is enabled in $$PWD/qwtconfig.pri, but Qt has not been built with svg support")
        }
    }
    else {
        QT += svg
...
contains(QWT_CONFIG, QwtOpenGL) {
   ...
   greaterThan(QT_MAJOR_VERSION, 4) {
        ...
        qtHaveModule(opengl) {
            ...
            warning("QwtOpenGL is enabled in $$PWD/qwtconfig.pri, but Qt has not been built with opengl support")
        }
    }
    else {
        QT += opengl
